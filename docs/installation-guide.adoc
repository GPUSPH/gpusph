...
With version 6.0, we have introduced the possibility to run GPUSPH on CPU (_CPU backend_)
...
* the NVIDIA CUDA Software Development Kit, version 8 or higher, to enable the CUDA backend
  (build and run GPUSPH on NVIDIA GPUs);
...
----
git clone {url-github} # use {url-git} if you have enabled SSH access on GitHub
cd gpusph
----
...
See the link:user-guide.html[GPUSPH User Guide] for additional details
...
chrono:: enables (`1`) or disables (`1`) fluid/solid interaction through Project Chrono;
