...
\be
[f]^c(\vec{r}) = \int_\Omega f(\vec{r'}) w(|\vec{r}-\vec{r'}|,h) \td \vec{r'},
\label{e:sph:cint}
\en
...
\be \label{e:sph:gaussian}
  w(|\vec{r}-\vec{r}'|) = \dfrac{\alpha_{G}}{h^3} \left( e^{-q^2} - e^{-\left(\frac{3}{h}\right)^2} \right)
\en
where $\alpha_{G} = \frac{1}{\pi \left( \sqrt{\pi}-36e^{-9}\right)}$.
\be \label{e:sph:gradgaussian}
  w'(|\vec{r}-\vec{r}'|) = -2q\dfrac{\alpha_{G}}{h^3} \left( e^{-q^2} - e^{-\left(\frac{3}{h}\right)^2} \right)
\en

\todo{Complete quadratic kernel information}
...
\todo{Add moving bodies description}
