...
\item \cmd{tend} is the simulated time, in seconds
...
\begin{itemize}
\item \cmd{dt} is the first time step size
\item \cmd{dtadaptfactor} is the CFL coefficient, usually taken as 0.3
\item \cmd{buildneibsfreq} is the neighbor counting frequency, in terms of number of time steps
\item \cmd{nlexpansionfactor} is the factor increasing the area where we count the neighbor particles
\end{itemize}

\subsubsection{Physical parameters}
 
The fluid density is set through the \cmd{add_fluid}) function,
and the equation of state to be used by \cmd{set_equation_of_state}.
...
GeometryType (GT) are fluid, fixed boundary, open boundary, 
