...
`float3 const& set_gravity(const float3 gravity)` set the three spatial components of gravity

`float3 set_gravity(const float gravityX, const float gravityY, const float gravityZ)` set the three spatial components of gravity

`float3 set_gravity(const float gravityZ)` assume the gravity vertical and set only the vertical component.

`float3 const& get_gravity() const` Get the three spatial components of gravity

`float get_gravity_magnitude() const` Get the magnitude of gravity

=== Materials properties [[materials]]

==== Density

The density of a fluid can be specified while creating the new fluid using the function `size_t add_fluid(float rho)`. The function
returns the index of the new created fluid, that can be stored in a variable to be used for easier referencing in the case of multi-fluid simulation.
...
the user-specified value with the density (e.g. if the user selected the `DYNAMIC`
computationa viscosity option, and then uses `set_kinematic_visc`, the dynamic
viscosity will be computed by multiplying `nu` times the density of the specific
fluid). Because of this, <<density,`set_density`>> should not be called _after_
setting the viscosity, as this may lead to inconsistent results.

<!-- TODO this should probably be enforced in the code -->
...
set with `void set_visc_power_law(size_t fluid_idx, float n)` and retrieved with
...
<<rheology,rheologicalmodels>>), the exponential coefficient can be set with
`void set_visc_exponential_coeff(size_t fluid_idx, float n)` and retrieved with
...
DISPLAYWRITER:: #TODO# integration with Paraview Catalyst
...
Follows a list of the geometries available in GPUSPH.
...
(setting <<filltype,`FT_UNFILL`>>), erasing all particles located in its positive
...
(setting <<filltype,`FT_NOFILL`>> instead).
The FillType field can be omitted, then the default value `FT_NOFILL` will be assigned.
...
==== Cube
The `Cube` object is actually a parallelepiped, defined by an origin,
...
[source,cpp]
----
Cube mycube = Cube(Point(0, 0, 0),Vector(1.6, 0, 0),Vector(0, 0.67, 0), Vector(0, 0, 0.4));
----
This box has a corner located at the origin of the domain, with `(x, y, z) = (0, 0, 0)`,
and three vectors from this point describe the cube,
which happens to be 1.6 m long in the x direction, 0.67 m long in the
y direction, and 0.4 in the z direction.

So far we have only defined the cube `experiment\-box`, we have
...
**TODO** use the definition of cube that we have in problem_api1, instead?
...
----
This command adds an empty (<<filltype,FT_BORDER>>) cylinder of fixed solid boundary ([GT_FIXED_BOUNDARY](geometrytype)) placed in
...
[source,cpp]
----
GeometryID ProblemAPI<1>::addCylinder(const GeometryType otype, const FillType ftype, const Point &origin, const double outer_radius, const double height)
----
...
----
This command adds an empty (<<filltype,FT_BORDER>>) sphere of fixed solid boundary ([GT_FIXED_BOUNDARY](geometrytype)) placed in
the point with coordinates `(x,y,z) = (0,0,0), with radius 1 m.
Syntax:
[source,cpp]
----
GeometryID ProblemAPI<1>::addSphere(const GeometryType otype, const FillType ftype, const Point &origin, const double radius)
----
...
Example 1: cube with cylindrical hole
...
Example 2: We want to make a hole on a solid cylinder and fill it with fluid
[source,cpp]
----
// Reference geometries position from their corner
setPositioning(PP_CORNER);
...
declared in the See the `problem_name.h` file. See the
[getting started guide]({{ site.baseurl }}/documentation/get-started-guide) for further detail.
...
Example of usage:
We want to assign the value to the variable `double viscosity`at runtime.
[source,cpp]
----
// Here we are inside the constructor of the problem
const double viscosity = get_option("visc", 1e-6);
----

To assign a value to `viscosity` at runtime we will execute GPUSPH as:
[source,sh]
----
./GPUSPH --visc 1e-2
----
