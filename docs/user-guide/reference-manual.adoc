...
----
  SETUP_FRAMEWORK(
    ...
    extforce_type<UNIFORM_EXTERNAL_FORCE>,
...
==== External force type (`extforce_type`) [[extforce_type,extforce-type]]

This is used to specify how the external body force (e.g. gravity) is defined in space:
[horizontal]
`UNIFORM_EXTERNAL_FORCE`:: the external body force is uniform in space (default)
`CUSTOM_EXTERNAL_FORCE`:: the external body force at each point in space is determined by a user-define, problem-specific function.

NOTE: variability of gravity / external forces in _time_ is controlled by the `gcallback`
boolean member in the simulations parameters (see the <<Seiche>> test case as an example).

****
#TODO#: Document the `simparams()->gcallback` setting and the `g_callback()` function.
****

If the external force type is set to `CUSTOM_EXTERNAL_FORCE`, then the test case must define
an `external_force` device function with the following structure:
[source,cpp]
----
template<typename FP /* ForcesParams */, typename P /* ParticleData */>
__device__ __forceinline__
enable_if_t<FP::extforce == CUSTOM_EXTERNAL_FORCE, float3>
external_force(FP const& params, P const& pdata)
{
    float3 g;
    /* HERE: compute g based pdata.gridPos and pdata.pos or any other parameter */
    return g;
}
----

****
#TODO#: This syntax isn't very friendly, ideally this should be simplified a bit.
****
...
==== setNumBoundaryLayers [[setNumBoundaryLayers,setDynamicBoundariesLayers]]

[source,cpp]
----
void setDynamicBoundariesLayers(const uint numLayers) /* DEPRECATED */
...
==== getNumBoundaryLayers [[getNumBoundaryLayers,getDynamicBoundariesLayers]]

[source,cpp]
----
uint getDynamicBoundariesLayers() /* DEPRECATED */
...
Returns 1 for single-layer boundary models (e.g. Lennard–Jones).
...
include::warn-no-world-in-constructor.adoc[]
...
The provided value can be overridden at runtime using the `--deltap` command-line option.
...
=== Interactions
When geometries intersect each other an erase operation can be set.
This operation is meant to avoid filling the intersection region multiple times,
but can also as a way to obtain complex geometries as the subtraction of elementary ones.
...
NOTE: particles belonging to moving or floating objects are never erased, but see the <<trim,trimming>>
command for an alternative approach.

A geometry with erase operations will erase particles only from the geometries previously defined.

It is also possible to set the intersection type
[source,cpp]
----
setIntersectionType(const GeometryID id, const IntersectionType i_type);
----
where the possible values for the intersection type are:
[horizontal]
IT_NONE:: no intersection is computed
IT_INTERSECT:: particles outside the geometry will be removed
IT_SUBTRACT:: particles inside the geometry will be removed
...
To define complex shapes, or to “carve out”parts of a moving or floating object,
one can use the `trim()` command:
[source,cpp]
----
trim(const GeometryID id, const GeometryID trimmer);
----

This will use the geometry `trimmer` to carve out particles from the filling of geometry `id`.
In this case, the `EraseOperation` of `trimmer` is ignored, but the `IntersectionType`
and unfill radius are still taken into account.

NOTE: the `trimmer` geometry gets disabled automatically, so it cannot be used for
filling or global unfilling operations after being used as an argument to a `trim` command.
The `clone()` command can be used to pass a copy of it if the original geometry is intended
for global usage as well, e.g.:
[source,cpp]
----
trim(id, clone(trimmer));
----

TIP: a trimmed geometry can be used to trim other geometries as well.
This makes it easier to define more complex geometries where only part of another shape
should be removed. See the <<<SlidingWedge>>> test case for example usage.
...
declared in the `problem_name.h` file.

The auxiliary functions are defined as a method for the class associated to the problem,
so the definition should begin with `problem_name::auxiliary_function_name`.

=== moving_body_dynamics_callback

This function is used to assign a prescribed motion to moving bodies
(i.e. bodies defined with the `GT_MOVING_BODY` geometry type).
It gets called for each moving body, twice per integration step
(once during the predictor and once during the corrector)
between the forces computation step and the integration step.
...
The forces and torques are only available if the body had force-feedback enabled
(`enableFeedback(geometry_id)`) and only account for contributions from the fluid
External forces such as gravity must be taken into account in the callback, if so desired.

Three copies of the kinematic data are avalable:
`initial_kdata` holds the initial state of the body (at simulation start);
`kdata0` holds the state of the body at the beginning of the time step;
`kdata` holds the state of the body at the last half integration step
(and is thus equal to `kdata0` during the predictor, and equal to the predictor output during the corrector),
and should be overwritten with the new state by the callback,
together with the acceleration data `adata` and the translation and rotation increments `dx` and `dr`.

NOTE: the linear and rotation increments `dx`, `dr` describe the difference between the new
(`kdata`) and old (`kdata0`) position of the center of mass and orientation of the body around it.
Although it may seem redundant having to specify both, this choice ensures that all data can be computed
with the highest possible accuracy.

****
#TODO# descrption of the `KinematicData` and `AccelerateData` structures
****

The recommended way to prescribe the object motion is to define or calculate the linear and angular acceleration,
(`adata.avel_dt`, `adata.lvel_dt`),
using them to compute the new linear and angular velocity
(`kdata.avel`, `kdata.lvel`),
from these compute the new rotation increment (`dr`) around the center of rotation,
as well as the new linear translation (`dx`) of the center of rotation,
and finally apply `dx` to `kdata0` to obtain the new position of the center of rotation.
This ensures that all fields are properly and consistently initialized.

.Imposing linear and angular motions to different objects
[example]
====
In this example we show an example usage of `moving_body_dynamics_callback` for a sample test case named `MyCase`.
We will impose a rotational motion to the moving object with index `0`
and a translational motion to the moving object with index `1`.

****
#TODO FIXME# it doesn't really do what it's supposed to be doing.
****
...
----
void
MyCase::moving_body_dynamics_callback(const uint index, ObjectPtr object,
	const double t0, const double t1, const double dt, const int step,
	float3 const& force, float3 const& torque,
	KinematicData const& initial_kdata, KinematicData const& initial_kdata,
	KinematicData& kdata, AccelerateData& adata, double3& dx, EulerParameters& dr)
{
...
=== moving_bodies_callback (**DEPRECATED**) [[moving-bodies-callback]]

This function was used to assign the motion of moving bodies before GPUSPH v6.
It has been deprecated in favor of <<<moving_body_dynamics_callback>>,
