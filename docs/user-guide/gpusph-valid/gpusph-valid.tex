...
\item \textbf{Description of the case}: this is a schematic case of a laminar channel flow with an inflow and an outflow boundary.
  The channel has a rectangular section of size $1m x 1m$ and a length of $4m$.

  We consider a simple 3-dimensional rectangular channel which is
  filled with fluid particles, and subjected to a gravity field force acting in a parallel and
  opposite manner with respect to the bottom's normal. The case is non-dimensionalized by using the water depth $h$
  and other physical parameters. The non-dimensional parameters and variables are as seen below:   
  \begin{equation}
    \begin{array}{l}
...
\subsection{Setup in Salome}
For this test-case, the geometrical setup only contains the channel and the initial free-surface shape.
We will not use any repacking so it is not necessary to include the free-surface in the main boundary shell.
In order to define the inflow and outflow boundaries, it will be necessary to define groups of faces
corresponding to these two areas in the GEOM object of the channel. The steps below are performed:
\begin{enumerate}
\item \textbf{Create the geometry using the Salome GEOM module}\smallskip\\
  \includegraphics[scale=0.55,trim={0cm 36.5cm 50cm 1cm}, clip]{../fig/SPHERIC2/Salome/geom_switch.png}
  \begin{itemize}
