...
EnergyGenerator::EnergyGenerator(const Options &options) : Problem(options)
{
	...
	m_size = make_float3(9.0f, 2.0f, 1.0f);
...
int EnergyGenerator::fill_parts()
{
	...
	Point p1 = Point(h_length + slope_length/(cos(beta)*10), width/2, 0);
	Point p2 = Point(h_length + slope_length/(cos(beta)*5), width/2, 0);

	cyl1 = Cylinder(p1, Vector(.025, 0, 0), Vector(0, 0, height));
	cyl1.SetPartMass(m_deltap, m_physparams.rho0[0]);
	cyl1.FillBorder(boundary_parts, br, true, true);
	
	torus = Torus(p1 + Vector(0, 0, height + 0.2), Vector(0, 1, 1), 0.3 , 0.1);
	...
	sphere.Fill(boundary_parts, br);
	
	cyl2 = Cylinder(p2, 0.3, height, EulerParameters(0.0, M_PI/2.0, 0.0));
	cyl2.SetPartMass(m_deltap, m_physparams.rho0[0]);
	cyl2.FillBorder(boundary_parts, br);
	//cyl2.Fill(boundary_parts, br);

	cone = Cone(p1 + Vector(4.5, 0, height + 0.2), 0.4 , 0.1, Vector(0, 0, 1.0));
	cone.FillBorder(boundary_parts, br);
	
	double theta = M_PI/4.0;
	EulerParameters ep(M_PI/3.0, -M_PI/7.0, M_PI/5.0);
	//EulerParameters ep(M_PI/3.0, 0, 0);
	ep.ComputeRot();
	cube1 = Cube(Point(m_origin + 0.5*m_size), 0.5, 0.5, 0.5, ep);
	cube1.FillBorder(boundary_parts, br);
	
	//cube2 = Cube(Point(m_origin + 0.5*m_size), 0.5*ep.Rot(Vector(1, 0, 0)), 
	//	0.5*ep.Rot(Vector(0, 1, 0)), 0.5*ep.Rot(Vector(0, 0, 1)));
	...
	cube1.Unfill(parts, br);
	cyl1.Unfill(parts, br);
	cyl2.Unfill(parts, br);
