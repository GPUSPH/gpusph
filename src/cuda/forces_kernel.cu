...
SPSstressMatrixDevice(sps_params<kerneltype, boundarytype, simflags> params)
{
	...
	for (idx_t i = 0; i < d_neiblist_end; i += d_neiblist_stride) {
		...
		if ( r < params.influenceradius ) {
			const float f = F<kerneltype>(r, params.slength)*relPos.w/absolute_density(relVel.w,fluid_num(neib_info));	// 1/r ∂Wij/∂r Vj
	...
	write_sps_turbvisc<simflags & SPSK_STORE_TURBVISC>::with(params, index, nu_SPS);
        ...
        float abs_rho = absolute_density(vel.w,fluid_num(info));
	...
	const	float	influenceradius)
{
	...
	vel.w = relative_density(vel.w,fnum);
	velArray[index] = vel;
				...
				const float		influenceradius)
{
	...
	float temp2 = temp1/absolute_density(vel.w,fluid_num(info)) ;
	...
	for_each_neib2(PT_FLUID, (boundarytype == DYN_BOUNDARY ? PT_BOUNDARY : PT_NONE),
		...
		const float neib_rho = absolute_density(tex1Dfetch(velTex, neib_index).w,fluid_num(neib_info));
	...
	vel.w = relative_density(temp1/temp2,fluid_num(info));
	newVel[index] = vel;
}
			...
			const float		influenceradius)
{
	...
	mls.xx = W<kerneltype>(0, slength)*pos.w/absolute_density(vel.w,fluid_num(info));
...
#endif
        vel.w = relative_density(vel.w,fluid_num(info));
	newVel[index] = vel;
}
/************************************************************************************************************/
