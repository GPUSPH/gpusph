...
/*! \file
 * Interface of the CPU-based device worker
 */
...
// In CPUWoker we implement as "private" all functions which are meant to be called only by the simulationThread().
...
public:
	...
	CPUWorker(GlobalData* _gdata, devcount_t _devnum);
	~CPUWorker();
