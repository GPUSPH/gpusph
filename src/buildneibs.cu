...
buildNeibsList4(uint*				neibsList,
				...
				const bool			periodicbound)
{
	const int numThreads = min(BLOCK_SIZE_BUILDNEIBS4, numParticles);
	const int numBlocks = (int) ceil(numParticles / (float) numThreads);
	...
	buildNeibsListDevice4<<< numBlocks, numThreads >>>(pos, neibsList, gridSize, cellSize, worldOrigin, numParticles, gridCells, sqinfluenceradius);
	// check if kernel invocation generated an error
	CUT_CHECK_ERROR("Kernel execution failed");
...
buildNeibsList2(uint*			neibsList,
				...
				bool			periodicbound)
{
	int numThreads = BLOCK_SIZE_BUILDNEIBS2;
	dim3 grid_size;
	grid_size.x = gridSize.x;
	grid_size.y = gridSize.y;
	grid_size.z = gridSize.z;
	...
	buildNeibsListDevice2<<< grid_size, numThreads >>>(pos, neibsList, gridSize, cellSize, worldOrigin, numParticles, gridCells, sqinfluenceradius);
