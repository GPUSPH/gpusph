...
WaveTank::WaveTank(const Options &options) : Problem(options)
{
	...
	m_physparams.mbamplitude = 0.4055f;   //At what z-level is this defined?????
	m_physparams.mbomega = 10.68f;
	m_physparams.stroke = 0.2442f;   //(BDR)
	m_physparams.paddle_h_SWL = .15f;  //(BDR)
...
#define MK_par 2
	...
	m_physparams.MK_beta = MK_par;
     ...
     if (wmakertype == 0 ){
        ...
        m_physparams.mbamplitude = 0.4055f;   //At what z-level is this defined?????
	    float koh =(m_physparams.mbomega*m_physparams.mbomega*H/g);
	    float wvnum= koh*pow(1.0-exp(-pow(koh,1.25)),-0.4)/H;
	    float theta = 0.0;
        std::cout << "Wave number: " <<  wvnum << "\n";
	    std::cout << "Wave period: " << 6.283/m_physparams.mbomega <<"\n";

	    m_physparams.mbksintheta = wvnum*sin(theta*3.1415927/180.);
	    m_physparams.mbphase = 0.0f;
	    m_physparams.mborigin = make_float3(0.13f, m_physparams.r0, -0.1344);
	    m_physparams.mbv = make_float3(0.0f, 0.0f, .5f);
	    m_physparams.mbtstart = make_float3(0.0, 1500*m_simparams.dt,0.0f);
	    m_physparams.mbtend = make_float3(0.0, 1000.,fabs(2.5*H/m_physparams.mbv.z));
	    ...
	    m_Hoh = amplitude/Hdepth;
	    float kappa = sqrt((3*m_Hoh)/(4.0*Hdepth*Hdepth));
	    float cel = sqrt(g*(Hdepth + amplitude));
	    m_S = sqrt(16.0*amplitude*Hdepth/3.0);
	//  std::cout << "cel:  " << cel << "\n";
...
{
	float posx = m_physparams.mbamplitude;
	float arg = 2.0*((3.8 + m_Hoh)*(t/m_tau - 0.5) - 2.0*m_Hoh*((posx/m_S) - 0.5));
	posx = m_S*(1.0 + tanh(arg))/2.0;
	m_physparams.mbamplitude = posx;
	m_mbcallback.mbamplitude = posx;
	return m_mbcallback;
}


int WaveTank::fill_parts()
{   i_use_bottom_plane = 0; //1 for real plane
...
//	fluid.InnerFill(parts, m_deltap);
    ...
    if (wmakertype == 0) {
	   ...
	   paddle.SetPartMass(m_deltap*m_deltap*m_deltap*m_physparams.rho0[0]);
	 ...
	 } else
	 {
	   ...
	   experiment_box6.SetPartMass(m_deltap*m_deltap*m_deltap*m_physparams.rho0[0]);    //  moving boundary
	   experiment_box6.Fill(piston_parts, br, true);
        ...
        if (i_use_bottom_plane == 0) {
           ...
           experiment_box1.SetPartMass(m_deltap*m_deltap*m_deltap*m_physparams.rho0[0]);
...
//	std::cout << "m_physparams.rho0: " << m_physparams.rho0 << "\n";
	while (z < H) {
		...
		float x = paddle_origin(0) + (z - paddle_origin(2))*tan(m_physparams.mbamplitude)  + 1.0*r0/cos(m_physparams.mbamplitude);
...
void WaveTank::draw_boundary(float t)
{
	...
	if (wmakertype == 0) {
		/*  double alpha = m_physparams.mbamplitude*cos(m_physparams.mbomega*t);
	  paddle = Rect(paddle_origin, Vector(0, paddle_width, 0),
			Vector(paddle_length*sin(alpha), 0 , paddle_length*cos(alpha)));
	  paddle.GLDraw();
  ...
  //    float r0 = m_physparams.r0;
	  float posx =  m_physparams.mbamplitude;
//	  std::cout << "mb_amplitude: " << m_physparams.mbamplitude << "\n";
	  Rect actual_gate = Rect(Point(posx, 0, 0), Vector(0, paddle_width, 0),Vector(0, 0, paddle_length));
   	  actual_gate.GLDraw();
   	}
...
void WaveTank::copy_to_array(float4 *pos, float4 *vel, particleinfo *info)
{
	...
	std::cout << "Paddle parts: " << paddle_parts.size() << "\n";
