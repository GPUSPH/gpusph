	...
	float3	const&		cellSize)
{
	...
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(cuXCompleteSaExample::d_rho0, &physparams->rho0, MAX_FLUID_TYPES*sizeof(float)));
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(cuXCompleteSaExample::d_bcoeff, &physparams->bcoeff, MAX_FLUID_TYPES*sizeof(float)));
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(cuXCompleteSaExample::d_gammacoeff, &physparams->gammacoeff, MAX_FLUID_TYPES*sizeof(float)));
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(cuXCompleteSaExample::d_sscoeff, &physparams->sscoeff, MAX_FLUID_TYPES*sizeof(float)));
}
