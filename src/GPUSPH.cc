...
bool GPUSPH::runSimulation() {
	...
	while (gdata->keep_going) {
		...
		if (problem->get_simparams()->numODEbodies > 0) {
			...
			problem->ODE_bodies_timestep(gdata->s_hRbTotalForce, gdata->s_hRbTotalTorque, 2, gdata->dt, gdata->s_hRbGravityCenters, gdata->s_hRbTranslations, gdata->s_hRbRotationMatrices);
