...
# programmatically-generated files that define the template instances of the engines
# that we want to generate
INSTANCE_DIR=$(OPTSDIR)/instance

BUILDNEIBS_INSTANCE_FILE=$(INSTANCE_DIR)/buildneibs.i
EULER_INSTANCE_FILE=$(INSTANCE_DIR)/euler.i
FORCES_INSTANCE_FILE=$(INSTANCE_DIR)/forces.i
VISC_INSTANCE_FILE=$(INSTANCE_DIR)/viscengine.i
FILTERS_INSTANCE_FILE=$(INSTANCE_DIR)/filters.i
BOUND_INSTANCE_FILE=$(INSTANCE_DIR)/boundaryengine.i

INSTANCE_FILES =$(BUILDNEIBS_INSTANCE_FILE)
INSTANCE_FILES+=$(EULER_INSTANCE_FILE)
INSTANCE_FILES+=$(FORCES_INSTANCE_FILE)
INSTANCE_FILES+=$(VISC_INSTANCE_FILE)
INSTANCE_FILES+=$(FILTERS_INSTANCE_FILE)
INSTANCE_FILES+=$(BOUND_INSTANCE_FILE)

# we want these variables to be exported to the environment, because they will be used
# by the get-needed-instances script
export BUILDNEIBS_INSTANCE_FILE EULER_INSTANCE_FILE FORCES_INSTANCE_FILE
export VISC_INSTANCE_FILE FILTERS_INSTANCE_FILE BOUND_INSTANCE_FILE

# instance files are all generated by the same script, which may cause a problem
# with parallel builds. The solution is to make them depend all on a common file
# that is touched when generation is complete
INSTANCE_DONE=$(INSTANCE_DIR)/done
...
# names of the files holding the instances to be generated,
# stripping the $(OPTSDIR)
CUFLAGS += -DBUILDNEIBS_INSTANCE_FILE=$(BUILDNEIBS_INSTANCE_FILE:$(OPTSDIR)/%=%)
CUFLAGS += -DEULER_INSTANCE_FILE=$(EULER_INSTANCE_FILE:$(OPTSDIR)/%=%)
CUFLAGS += -DFORCES_INSTANCE_FILE=$(FORCES_INSTANCE_FILE:$(OPTSDIR)/%=%)
CUFLAGS += -DVISC_INSTANCE_FILE=$(VISC_INSTANCE_FILE:$(OPTSDIR)/%=%)
CUFLAGS += -DFILTERS_INSTANCE_FILE=$(FILTERS_INSTANCE_FILE:$(OPTSDIR)/%=%)
CUFLAGS += -DBOUND_INSTANCE_FILE=$(BOUND_INSTANCE_FILE:$(OPTSDIR)/%=%)
...
.PHONY: all run showobjs show snapshot expand deps docs test help instances
...
clean: cpuclean gpuclean instancesclean
...
# target: instancesclean - Clean autogenerated template engine instance selection
instancesclean:
	$(RM) -r $(INSTANCE_DIR)
...
# target: instances - Update the list of template engine instances to be generated
instances: $(INSTANCE_FILES)
	@true

$(INSTANCE_FILES): $(INSTANCE_DONE)
	@true

# build instance files. We put the script first so that we can call
# the build with just the dependencies
$(INSTANCE_DONE): scripts/get-needed-instances.sh $(PROBLEM_SRCS)
	$(call show_stage,SCRIPTS,Template instances)
	$(CMDECHO)$^
	$(CMDECHO)touch $@
