...
\begin{equation}
\tdv{\uvec{v}}{t} = -\frac{1}{\rho}\unabla p + \unabla \cdot (\nu
\unabla \otimes \uvec{v}) + \uvec{g},
\label{e:sph:ns}
\end{equation}
...
\begin{equation}
\tdv{\rho}{t} = - \rho \unabla \cdot \uvec{v}.
\label{e:sph:cont}
\end{equation}
In order to close the system of equations a weakly-compressible
formulation is chosen which uses an Equation of State given by
\begin{equation}
...
$\xi$ a parameter, equal to 7 in the case of water.
...
\label{e:sph:delta}
\end{equation}
where $\delta$ is the Dirac delta function. The continuous SPH
...
\begin{equation}
\|\unabla w(q,h)\| = \frac{\alpha_{\unabla W}}{h^5}\left(q-2\right)^3,
\label{e:sph:gradwendland}
\end{equation}
where $\alpha_{\unabla W} = 105/(128\pi)$ in 3-D.
...
\begin{equation}
[\unabla f]^c(r) = \int_\Omega \unabla f(s) w(\|r-s\|,h) \td s.
\label{e:sph:cint-gradf-start}
\end{equation}
Applying integration by parts yields
\begin{equation}
[\unabla f]^c(r) = - \int_\Omega f(s) \unabla_s w(\|r-s\|,h) \td s +
\int_\Omega \unabla (f(s) w(\|r-s\|,h)) \td s.
\label{e:sph:cint-intpart}
\end{equation}
As $w$ is symmetric the kernel is antisymmetric and thus
\begin{equation}
\unabla_s w(\|r-s\|,h) = - \unabla_r w(\|r-s\|,h).
...
\begin{equation}
[\unabla f]^c(r) = \int_\Omega f(s) \unabla_r w(\|r-s\|,h) \td s -
...
\begin{equation}
[\unabla f]_a = \sumF V_b f_b \unabla_a w_{ab},
...
\begin{equation}
\Grad_a(f) = [\unabla f]_a + [\unabla f_a]_a,
...
\begin{equation}
\Div_a(\uvec{G}) = [\unabla \uvec{G}]_a - [\unabla \uvec{G}_a]_a.
\label{e:sph:div-def}
\end{equation}
This allows to write the gradient and divergence as
\begin{equation}
\Grad_a(f) = \sumF V_b (f_b + f_a) \unabla_a w_{ab},
\label{e:sph:grad}
\end{equation}
and
\begin{equation}
\Div_a(\uvec{G}) = \sumF V_b (\uvec{G}_b - \uvec{G}_a) \cdot \unabla_a w_{ab},
\label{e:sph:div}
\end{equation}
respectively.
...
The goal of this section is to derive a discretization for $\unabla f
\unabla \otimes \uvec{G}$. To approximate the interior gradient, a
Taylor Series of $\uvec{G}$ can be used as follows
\begin{equation}
\uvec{G}_b = \uvec{G}_a - (\unabla_a \otimes \uvec{G})^T \cdot
...
\begin{equation}
\unabla_a w_{ab} = \frac{\uvec{r}_{ab}}{\|\uvec{r}_{ab}\|} \|\unabla_a
w_{ab}\|.
\label{e:sph:rad-sym}
\end{equation}
...
\begin{equation}
\unabla \cdot (f \unabla \otimes \uvec{G}) = \sumF V_b (f_b \unabla_b
\otimes \uvec{G} + f_a \unabla_a \otimes \uvec{G})
\frac{\uvec{r}_{ab}}{\|\uvec{r}_{ab}\|} \|\unabla_a w_{ab}\|,
\end{equation}
which, with Eqs. \eqref{e:sph:grad-finitediff},
\eqref{e:sph:grad-finitediff-b} and \eqref{e:sph:rad-sym}, can be used to
define the Laplacian
\begin{equation}
\Lap_a(f, \uvec{G}) = \unabla \cdot (f \unabla \otimes \uvec{G}) = \sumF
V_b (f_b + f_a) \uvec{G}_{ab} \frac{1}{\|\uvec{r}_{ab}\|} \|\unabla_a w_{ab}\|.
\label{e:sph:lap}
\end{equation}
